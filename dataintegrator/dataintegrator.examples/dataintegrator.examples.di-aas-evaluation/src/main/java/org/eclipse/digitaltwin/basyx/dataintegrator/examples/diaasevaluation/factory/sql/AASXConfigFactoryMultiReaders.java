...
public class AASXConfigFactoryMultiReaders {
	...
	private AASXStencilFactoryMultiReader aasxStencilFactory;

	public AASXConfigFactoryMultiReaders(AASXStencilFactoryMultiReader aasxStencilFactory) {
	...
	public AasEnv create() {
		...
		Submodel readerConfig = createReaderConfig();
		...
		configAAS.addSubmodelReference(
				new Reference(new Key(KeyElements.SUBMODEL, true, "readerEvalSMId", KeyType.CUSTOM)));
		configAAS.addSubmodelReference(
		...
		submodelList.addAll(Arrays.asList(readerConfig, processorConfig, writerConfig, jobConfig));
	...
	private Submodel createJobConfig() {
		...
		SubmodelElementCollection readers = createReaderReferences();
		...
		jobSM.addSubmodelElement(readers);
...
//		writers.addSubmodelElement(writer0);
	...
	private SubmodelElementCollection createReaderReferences() {
		...
		ReferenceElement reader0 = new ReferenceElement("reader0");
		reader0.setSemanticId(new Reference(
				new Key(KeyElements.GLOBALREFERENCE, true, "basyx-di:connector:config:reader", KeyType.IRI)));
		reader0.setValue(new Reference(new Key(KeyElements.SUBMODEL, true, "readerEvalSMId", KeyType.CUSTOM)));
	...
	private Submodel createWriterConfig() {
		...
		StringBuilder prevIdShortPath = new StringBuilder(
				DEFAULT_IDSHORT_PREFIX + "/" + AASXStencilFactoryMultiReader.SME_ID_PREFIX + 0);
		...
		while (nextId <= aasxStencilFactory.getLevel()) {
			prevIdShortPath.append("/" + AASXStencilFactoryMultiReader.SME_ID_PREFIX + nextId);
		...
		SubmodelElementCollection lastPropertyConfig = createSMCConfig(SMC_CONFIG_ID_PREFIX + nextId,
				prevIdShortPath + "/" + "legacySysProperty", "language");
	...
	private Submodel createProcessorConfig() {
		...
		processorQualifier
				.setValue("org.eclipse.digitaltwin.basyx.dataintegrator.connector.sqlprocessor.processor.SqlProcessor");
	...
	private Submodel createReaderConfig() {
		Submodel readerSM = new Submodel("readerEvalSMIdShort", new CustomId("readerEvalSMId"));
		...
		SubmodelElementCollection sqlConnectionProperties = createSqlConnPropSMC();
	...
	private SubmodelElementCollection createValueMapSMC() {
		...
		Property language = new Property("language", "LANG");
	...
	private SubmodelElementCollection createSqlConnPropSMC() {
		...
		Property sqlQuery = new Property("sqlQuery", "SELECT DISTINCT LANG FROM evaluation WHERE eval_id='?'");
		databaseName.setValueType(ValueType.String);
