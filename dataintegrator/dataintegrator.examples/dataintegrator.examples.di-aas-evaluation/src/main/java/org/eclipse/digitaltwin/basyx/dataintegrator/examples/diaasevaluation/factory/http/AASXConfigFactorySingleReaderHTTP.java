...
public class AASXConfigFactorySingleReaderHTTP {
	...
	public AASXConfigFactorySingleReaderHTTP(AASXStencilFactorySingleReaderHTTP aasxStencilFactory) {
...
//		}
	...
	private Submodel createReaderConfig(int index, String apiUrl) {
		...
		Property mapperType = new Property("mapperType", "MultiValueMapper");
		mapperType.setValueType(ValueType.String);
		
		int countOfValueMap = aasxStencilFactory.getLevel();

		SubmodelElementCollection valueMap = createValueMapSMC(countOfValueMap);
	...
	private SubmodelElementCollection createValueMapSMC(int countOfValueMap) {
		SubmodelElementCollection valueMap = new SubmodelElementCollection("valueMap");
		
		Property address = new Property();
		address.setIdShort("addr");
		address.setValueType(ValueType.String);
		
		StringBuilder multiExpressionBuilder = new StringBuilder("[");
		
		for (int i = 1; i <= countOfValueMap; i++) {
			multiExpressionBuilder.append("\"$.address.address" + i + "\",");
		}
		multiExpressionBuilder.deleteCharAt(multiExpressionBuilder.length() - 1);
		
		multiExpressionBuilder.append("]");
		
		address.setValue(multiExpressionBuilder.toString());
